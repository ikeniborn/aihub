...
aihub скачивает GGUF-файлы Ollama из OCI-реестра напрямую и сохраняет их локально.
Чтобы использовать эти файлы в `ollama run`, нужно зарегистрировать их через
`ollama create FROM <path>`. После регистрации локальный GGUF можно удалить —
Ollama хранит собственную копию в `$OLLAMA_MODELS/blobs/`.

---

### Через веб-интерфейс (рекомендуется)

В вкладке **Мои модели** у каждой скачанной Ollama-модели есть кнопка **→Ollama**.
Нажатие запускает `ollama create FROM <path>` на сервере и регистрирует модель.

Если модель ещё не скачана локально, но есть в S3, отображается кнопка **↓→Ollama**:
она сначала скачивает файл из S3, затем автоматически регистрирует в Ollama.

```bash
make ui   # открыть веб-интерфейс → кнопка →Ollama в строке модели
```
...
```bash
# Скачать Ollama-модели и сразу зарегистрировать в Ollama
python scripts/download_models.py --register-ollama

# Зарегистрировать уже скачанные модели (скачивание будет пропущено)
python scripts/download_models.py --register-ollama
# (скачанные модели получат статус SKIP, но регистрация выполнится для них тоже)
```

Требования: `ollama` должен быть установлен и доступен в `PATH`.

---

### Bash-скрипт (для массовой регистрации вручную)

```bash
# Зарегистрировать все GGUF из папки в Ollama
...
# После проверки — удалить локальные копии (блобы уже в ollama/blobs/)
# rm /path/to/models/misc/*.gguf
```

> `ollama_model` в `models.yaml` имеет вид `model:tag` (например, `qwen3:7b`) — используйте
> его как `$name` для сохранения правильного имени в Ollama.

---

### Управление дисковым пространством

| Состояние | Место на диске |
|-----------|---------------|
| Только в aihub (`models/`) | 1× размер GGUF |
| После `ollama create` (без удаления) | 2× размер GGUF |
| Удалить локальный GGUF, оставить в Ollama | 1× размер GGUF |
| Удалить локальный GGUF, оставить в S3 + Ollama | 1× размер GGUF |

После регистрации в Ollama локальный файл в `models/` можно удалить через
кнопку **✕** в веб-интерфейсе (выбрав «Только локально»).

```bash
ollama list          # модель зарегистрирована
